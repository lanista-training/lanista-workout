...
import { SLIDE_HORIZONTAL, SLIDE_VERTICAL, FADE_VERTICAL, CUBE, FADE, EXPAND } from './animationTypes';
    ...
    if (transitionType === PUSH && screenIndex === 1) {
      return {
        height: animation.interpolate({
          ...
          outputRange: [height, target ? target.height : height, height],
        }),
        width: animation.interpolate({
          inputRange: inputRange,
          outputRange: [width, target ? target.width : width, width],
        }),
        top: animation.interpolate({
          inputRange: [-height, 0, height],
          outputRange: [0, target ? target.y : (height/2), 0],
        }),
        left: animation.interpolate({
          inputRange: [-height, 0, height],
          outputRange: [0, target ? target.x/2 : (width/2), 0],
        }),
        ...baseStyle,
      };
    } else if (transitionType === POP && screenIndex === 1) {
      return {
        height: animation.interpolate({
          inputRange: inputRange,
          outputRange: [target ? target.height/2 : height, height, target ? target.height/2 : height],
        }),
        width: animation.interpolate({
          inputRange: inputRange,
          outputRange: [target ? target.width : width, width, target ? target.width : width],
        }),
        top: animation.interpolate({
          inputRange: [-height, 0, height],
          outputRange: [target ? target.y : height/2, 0, target ? target.y : height/2],
        }),
        left: animation.interpolate({
          inputRange: [-height, 0, height],
          outputRange: [target ? target.x/2 : width/2, 0, target ? target.x/2 : width/2],
        }),
        ...baseStyle,
      };
    }
